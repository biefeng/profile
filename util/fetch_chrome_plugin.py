...
def fetch_plugin_info_into_file():
    ...
    with open(chrome_data_dir + 'chrome_plugin.csv', mode="w") as sql_file:
...
def upload_crx_to_baidu_obs():
    with open(chrome_data_dir + "chrome_plugin.csv", mode='r') as csv_file:
        ...
        for row in reader:
            ...
            with open(fn, mode='rb') as file_data:
                ...
                # print(url)
                # break
        with open(chrome_data_dir + "chrome_plugin_url.csv", mode="w") as url_file:
            writer = csv.DictWriter(url_file, fieldnames=['id', 'name', 'short_desc', 'description', 'cover_image', "crx_url"], delimiter=r" ")
            writer.writeheader()
